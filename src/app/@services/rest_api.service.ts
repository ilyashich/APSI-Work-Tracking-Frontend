    ...
    return this.oauthService.getToken().pipe(
      concatMap(
        token => {
          ...
          const parsedToken = JSON.parse(stringToken);
          const tokenResult = parsedToken['access_token'];
          return this.http.put<Blob>(this.baseUrl + '/' + accessUrl + '/order/download',
            { 'uuid': documentId, 'language': language },
            this.initAccessTokenHeaders(tokenResult, 'blob')).pipe(
              timeout(this.requestTimeout),
              // ================
              flatMap(response => of({ item: response, error: null })),
    ...
    return this.oauthService.getToken().pipe(
      concatMap(
        token => {
          ...
          const parsedToken = JSON.parse(stringToken);
          const tokenResult = parsedToken['access_token'];
          return this.http.get<any>(url, this.initAccessTokenHeaders(tokenResult, undefined)).pipe(
            timeout(this.requestTimeout),
            // ================
            flatMap(response => of({ item: response, error: null })),
            // ================
    ...
    return this.oauthService.getToken().pipe(
      concatMap(
        token => {
          ...
          const parsedToken = JSON.parse(stringToken);
          const tokenResult = parsedToken['access_token'];
          return this.http.post<any>(url, request, this.initAccessTokenHeaders(tokenResult, undefined)).pipe(
            timeout(this.requestTimeout),
            // ================
            flatMap(response => of({ item: response, error: null })),
            // ================
    ...
    return this.oauthService.getToken().pipe(
      concatMap(
        token => {
          ...
          // -------------
          return this.http.put<any>(url, req, { headers }).pipe(
            timeout(this.requestTimeout),
            // ================
            flatMap(response => of({ item: response, error: null })),
            // ================
